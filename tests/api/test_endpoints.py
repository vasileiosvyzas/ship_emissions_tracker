...
def test_emissions_with_the_correct_ship_type():
    ...
    with client as cl:
        ...
        assert response["status-code"] == 200
        assert response["message"] == "OK"
        assert response["data"]["columns"] == ["Total CO₂ emissions [m tonnes]","Reporting Period","IMO Number"]
        assert len(response["data"]["data"]) != 0
...
def test_fuel_consumption_with_the_correct_ship_type():
    ...
    with client as cl:
        ...
        assert response["status-code"] == 200
        assert response["message"] == "OK"
        assert response["data"]["columns"] == ['Total fuel consumption [m tonnes]', 'Reporting Period', 'IMO Number']
        assert len(response["data"]["data"]) != 0
